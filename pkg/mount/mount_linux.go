...
func (m *Mounter) doMount(mounterPath string, mountCmd string, source string, target string, fstype string, options []string) error {
	...
	log.L().Debugw("mounting", "cmd", mountCmd, "arguments", mountArgs)
	command := exec.Command(mountCmd, mountArgs...)
	output, err := command.CombinedOutput()
	if err != nil {
		args := strings.Join(mountArgs, " ")
		log.L().Errorw("mount failed", "cmd", mountCmd, "arguments", mountArgs, "error", err)
...
func detectSystemd() bool {
	if _, err := exec.LookPath("systemd-run"); err != nil {
		log.L().Debug("Detected OS without systemd")
		return false
	}
	...
	if err != nil {
		log.L().Debug("Cannot run systemd-run, assuming non-systemd OS")
		log.L().Debugw("systemd-run failed", "error", err)
		log.L().Debugw("systemd-run", "output", string(output))
		return false
	}
	log.L().Debug("Detected OS with systemd")
	return true
}
...
func (mounter *Mounter) Unmount(target string) error {
	log.L().Debugw("Unmounting", "target", target)
...
func exclusiveOpenFailsOnDevice(pathname string) (bool, error) {
	...
	if !isDevice {
		log.L().Errorw("not referring to a device", "path", pathname)
		return false, nil
	}
...
func getDeviceNameFromMount(mounter Interface, mountPath, pluginDir string) (string, error) {
	...
	if err != nil {
		log.L().Debugw("GetMountRefs failed", "mountpath", mountPath, "error", err)
		return "", err
	}
	if len(refs) == 0 {
		log.L().Debugw("not mounted", "mountpath", mountPath)
	...
	for _, ref := range refs {
		if strings.HasPrefix(ref, basemountPath) {
			...
			if err != nil {
				log.L().Errorw("failed to get volume id from mount", "mountpath", mountPath, "error", err)
				return "", err
			}
			return volumeID, nil
		}
...
func (mounter *SafeFormatAndMount) formatAndMount(source string, target string, fstype string, options []string) error {
	...
	if !readOnly {
		...
		log.L().Debugw("Checking for issues with fsck", "device", source)
		...
		if err != nil {
			...
			case err == utilexec.ErrExecutableNotFound:
				log.L().Warn("'fsck' not found on system; continuing mount without running 'fsck'.")
			case isExitError && ee.ExitStatus() == fsckErrorsCorrected:
				log.L().Infow("errors were corrected by fsck", "device", source)
			case isExitError && ee.ExitStatus() == fsckErrorsUncorrected:
				return fmt.Errorf("'fsck' found errors but could not correct them: device %s, output %s", source, string(out))
			case isExitError && ee.ExitStatus() > fsckErrorsUncorrected:
				log.L().Errorw("fsck", "error", string(out))
			}
		}
	}

	// Try to mount the disk
	log.L().Debugw("Attempting to mount", "fstype", fstype, "source", source, "target", target)
	...
	if mountErr != nil {
		...
		if existingFormat == "" {
			...
			log.L().Infow("appears to be unformatted, attempting to format", "device", source, "fstype", fstype, "args", args)
			_, err := mounter.Exec.Run("mkfs."+fstype, args...)
			if err == nil {
				// the disk has been formatted successfully try to mount it again.
				log.L().Infow("successfully formatted (mkfs)", "device", source, "fstype", fstype, "target", target)
				return mounter.Interface.Mount(source, target, fstype, options)
			}
			log.L().Errorw("format failed", "fstype", fstype, "device", source, "target", target, "options", options, "error", err)
			return err
		} else {
...
func (mounter *SafeFormatAndMount) GetDiskFormat(disk string) (string, error) {
	...
	log.L().Debugf("Attempting to determine if disk %q is formatted using blkid with args: (%v)", disk, args)
	dataOut, err := mounter.Exec.Run("blkid", args...)
	output := string(dataOut)
	log.L().Debugf("Output: %q, err: %v", output, err)
	...
	if err != nil {
		...
		log.L().Errorw("could not determine if disk is formatted", "disk", disk, "error", err)
		return "", err
	}
	...
	if len(pttype) > 0 {
		log.L().Debugf("Disk %s detected partition table type: %s", disk, pttype)
...
func doMakeRShared(path string, mountInfoFilename string) error {
	...
	if shared {
		log.L().Debugf("Directory %s is already on a shared mount", path)
		return nil
	}

	log.L().Debugf("Bind-mounting %q with shared mount propagation", path)
...
func prepareSubpathTarget(mounter Interface, subpath Subpath) (bool, string, error) {
	...
	if !notMount {
		...
		log.L().Debugw("skipping bind-mounting: already mounted", "subpath", bindPathTarget)
...
func doBindSubPath(mounter Interface, subpath Subpath) (hostPath string, err error) {
	...
	log.L().Debugw("doBindSubPath", "subpath", subpath.Path, "newpath", newPath, "volumepath", subpath.VolumePath)
	...
	defer func() {
		...
		if !success {
			log.L().Debugf("doBindSubPath() failed for %q, cleaning up subpath", bindPathTarget)
			if cleanErr := cleanSubPath(mounter, subpath); cleanErr != nil {
				log.L().Errorw("clean subpath", "subpath", bindPathTarget, "error", cleanErr)
			}
		}
	}()
	...
	// Do the bind mount
	options := []string{"bind"}
	log.L().Debugw("bind mounting", "source", mountSource, "target", bindPathTarget)
	if err = mounter.Mount(mountSource, bindPathTarget, "" /*fstype*/, options); err != nil {
		return "", fmt.Errorf("error mounting %s: %s", subpath.Path, err)
	}
	success = true

	log.L().Debugw("bound SubPath", "subpath", subpath.Path, "target", bindPathTarget)
	return bindPathTarget, nil
}
...
func doCleanSubPaths(mounter Interface, podDir string, volumeName string) error {
	...
	log.L().Debugf("Cleaning up subpath mounts for %s", subPathDir)
	...
	for _, containerDir := range containerDirs {
		if !containerDir.IsDir() {
			log.L().Debugf("Container file is not a directory: %s", containerDir.Name())
			continue
		}
		log.L().Debugf("Cleaning up subpath mounts for container %s", containerDir.Name())
		...
		log.L().Debugw("removed", "containerpath", fullContainerDirPath)
	}
	...
	log.L().Infow("removed", "subpath", subPathDir)
	...
	log.L().Infow("removed", "podsubpath", podSubPathDir)
	return nil
}
...
func doCleanSubPath(mounter Interface, fullContainerDirPath, subPathIndex string) error {
	...
	log.L().Debugf("Cleaning up subpath mounts for subpath %v", subPathIndex)
	...
	if !notMnt {
		...
		log.L().Debugw("unmounted", "path", fullSubPath)
	}
	...
	log.L().Debugw("removed", "path", fullSubPath)
	return nil
}
...
func removeEmptyDirs(baseDir, endDir string) error {
	...
	for curDir := endDir; curDir != baseDir; curDir = filepath.Dir(curDir) {
		...
		if err != nil {
			if os.IsNotExist(err) {
				log.L().Debugw("doesn't exist, skipping", "dir", curDir)
				continue
			}
		...
		if os.IsExist(err) {
			log.L().Debugw("not empty, not removing", "dir", curDir)
			break
		} else if err != nil {
			return fmt.Errorf("error removing directory %q: %v", curDir, err)
		}
		log.L().Debugw("removed", "dir", curDir)
	}
	return nil
}
...
func doSafeMakeDir(pathname string, base string, perm os.FileMode) error {
	log.L().Debugf("Creating directory %q within base %q", pathname, base)
	...
	if err == nil {
		...
		if s.IsDir() {
			...
			log.L().Debugf("Directory %s already exists", pathname)
			return nil
		}
	...
	log.L().Debugf("%q already exists, %q to create", fullExistingPath, filepath.Join(toCreate...))
	...
	defer func() {
		if parentFD != -1 {
			if err = syscall.Close(parentFD); err != nil {
				log.L().Debugf("Closing FD %v failed for safemkdir(%v): %v", parentFD, pathname, err)
			}
		}
		if childFD != -1 {
			if err = syscall.Close(childFD); err != nil {
				log.L().Debugf("Closing FD %v failed for safemkdir(%v): %v", childFD, pathname, err)
			}
		}
	}()
...
func findExistingPrefix(base, pathname string) (string, []string, error) {
	...
	defer func() {
		if err = syscall.Close(fd); err != nil {
			log.L().Debugf("Closing FD %v failed for findExistingPrefix(%v): %v", fd, pathname, err)
		}
	}()
...
func doSafeOpen(pathname string, base string) (int, error) {
	...
	defer func() {
		if parentFD != -1 {
			if err = syscall.Close(parentFD); err != nil {
				log.L().Debugf("Closing FD %v failed for safeopen(%v): %v", parentFD, pathname, err)
			}
		}
	}()
	...
	defer func() {
		if childFD != -1 {
			if err = syscall.Close(childFD); err != nil {
				log.L().Debugf("Closing FD %v failed for safeopen(%v): %v", childFD, pathname, err)
			}
		}
	}()
	...
	for _, seg := range segments {
		...
		log.L().Debugw("opening", "path", currentPath)
