...
func (m *execMounter) doExecMount(source, target, fstype string, options []string) error {
	log.L().Debugw("exec mounting", "source", source, "target", target, "fstype", fstype, "options", options)
	mountArgs := makeMountArgs(source, target, fstype, options)
	output, err := m.exec.Run("mount", mountArgs...)
	log.L().Debugw("exec mounted", "args", mountArgs, "error", err, "output", string(output))
	if err != nil {
...
func (m *execMounter) Unmount(target string) error {
	...
	if err == nil {
		log.L().Debugw("exec unmounted", "target", target, "output", string(outputBytes))
	} else {
		log.L().Errorw("failed to exec unmount", "target", target, "output", string(outputBytes), "error", err)
	}
