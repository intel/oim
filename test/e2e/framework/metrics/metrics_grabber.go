...
func NewMetricsGrabber(c clientset.Interface, ec clientset.Interface, kubelets bool, scheduler bool, controllers bool, apiServer bool, clusterAutoscaler bool) (*MetricsGrabber, error) {
	...
	if len(nodeList.Items) < 1 {
		log.L().Warn("Can't find any Nodes in the API server to grab metrics from")
	}
	...
	if !registeredMaster {
		...
		if clusterAutoscaler {
			log.L().Warn("Master node is not registered. Grabbing metrics from Scheduler, ControllerManager is disabled.")
		} else {
			log.L().Warn("Master node is not registered. Grabbing metrics from Scheduler, ControllerManager and ClusterAutoscaler is disabled.")
		}
	}
