...
func Init(metrics interface{}, factory Factory, globalTags map[string]string) {
	if err := InitWithError(metrics, factory, globalTags); err != nil {
		panic(err.Error())
	}
}

// InitWithError does the same as Init, but returns an error instead of
// panicking.
func InitWithError(m interface{}, factory Factory, globalTags map[string]string) error {
